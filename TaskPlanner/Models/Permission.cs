    ...
    public static class Permission
    {
        ...
        public static bool IsUserHasAccessToProject(string emailId, int projectId)
        {
            ...
            try
            {
                using (var context = new TaskPlannerEntities())
                {
                    var result = (from projectDetails in context.ProjectPermissions.Where(i => i.IsActive && i.ProjectId == projectId && i.EmailId == emailId)
                                  from resultDetails in context.Projects.Where(i => i.IsActive && i.ProjectId == projectDetails.ProjectId)
                                  select resultDetails).Count();
